...
interface TechnicalSupportContent {
  id: string
  section: string
  title?: string
  content: string
...
export default function AdminTechnicalSupportPage() {
  const { isAuthenticated } = useAdminAuth()
  const [contents, setContents] = useState<TechnicalSupportContent[]>([])
  const [isLoading, setIsLoading] = useState(true)
  const [isDialogOpen, setIsDialogOpen] = useState(false)
  const [editingContent, setEditingContent] = useState<TechnicalSupportContent | null>(null)
  ...
  const {
    ...
    resolver: zodResolver(technicalSupportContentSchema),
    defaultValues: {
      displayOrder: 0,
    },
  })
  ...
  const fetchContents = async () => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || ''}/api/technical-support-content`)
  ...
  const openEditDialog = (content: TechnicalSupportContent) => {
  ...
  const onSubmit = async (data: TechnicalSupportFormData) => {
    if (!isAuthenticated) {
    ...
    try {
      ...
      const url = editingContent
        ? `${process.env.NEXT_PUBLIC_API_URL || ''}/api/technical-support-content/${editingContent.id}`
        : `${process.env.NEXT_PUBLIC_API_URL || ''}/api/technical-support-content`
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || ''}/api/technical-support-content/${id}`,
        {
          method: 'DELETE',
          headers: {
            'X-CSRF-Token': csrfToken,
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Technical Support Content</h1>
          <p className="text-gray-600 mt-2">Manage Technical Support page content</p>
        </div>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              {editingContent
                ? 'Update Technical Support content section'
                : 'Add a new Technical Support content section'}
            </DialogDescription>
          </DialogHeader>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            <div>
              ...
              <Input
                id="section"
                {...register('section')}
                placeholder="e.g., hero, support-options, contact-info"
