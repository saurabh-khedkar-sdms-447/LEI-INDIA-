...
export default function AdminReturnsPage() {
  ...
  const fetchContents = async () => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || ''}/api/returns-content`)
  ...
  const onSubmit = async (data: ReturnsFormData) => {
    ...
    try {
      ...
      const url = editingContent
        ? `${process.env.NEXT_PUBLIC_API_URL || ''}/api/returns-content/${editingContent.id}`
        : `${process.env.NEXT_PUBLIC_API_URL || ''}/api/returns-content`
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || ''}/api/returns-content/${id}`,
        {
          method: 'DELETE',
          headers: {
            'X-CSRF-Token': csrfToken,
